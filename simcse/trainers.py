...
def Construct_Eval_Train_Dataset(datapath, tokenizer):
    datas = []
    ...
    text = text_a + text_b
    text_a = tokenizer(text, padding=True, truncation=True, return_tensors='pt')
    text_b = tokenizer(text, padding=True, truncation=True, return_tensors='pt')
    labels = torch.LongTensor(labels)
    tensors = list(text_a.values()) + list(text_b.values()) + [labels]
    return TensorDataset(*tensors)

def Construct_Eval_Test_Pos_Dataset(datapath, tokenizer):
    datas = []
    with open(datapath) as f:
        lines = f.readlines()
        for line in lines:
    ...
    text_b = [ele[1] for ele in datas]
    labels = [ele[2] for ele in datas]
    text_a = tokenizer(text_a, padding=True, truncation=True, return_tensors='pt')
    text_b = tokenizer(text_b, padding=True, truncation=True, return_tensors='pt')
    labels = torch.LongTensor(labels)
    tensors = list(text_a.values()) + list(text_b.values()) + [labels]
    return TensorDataset(*tensors)


def cal_train_align_uniform(model, dataloader, args=None):
    align_loss_list, uniform_loss_list, contrastive_loss_list = [], [], []
    encoder = getattr(model, model.encoder_name)
    for batch in dataloader:
        with torch.no_grad():
            batch = [ele.to(model.device) for ele in batch]

            outputs = encoder(input_ids=torch.cat((batch[0], batch[3]), dim=0), token_type_ids=torch.cat((batch[1], batch[4]), dim=0),
                                 attention_mask=torch.cat((batch[2], batch[5])), output_hidden_states=True, return_dict=True)
            embs = pooling(outputs, torch.cat((batch[2], batch[5]), dim=0), args)
            # outputs = model.bert(input_ids=batch[3], token_type_ids=batch[4], attention_mask=batch[5],
            #                 output_hidden_states=True, return_dict=True)
            # embb = pooling(outputs, batch[5], args)
            emba = embs[:int(embs.size(0)/2)]
            embb = embs[int(embs.size(0)/2):]
            z1 = F.normalize(emba, dim=-1)
            z2 = F.normalize(embb, dim=-1)
            align_loss = lalign(z1, z2, alpha=model.model_args.align_alpha, flood=model.model_args.align_flood)
            uniform_loss = lunif(z1, z2, t=model.model_args.uniform_t)
            ...
            uniform_loss_list.append(uniform_loss.item())
            contrastive_loss_list.append(con_loss.item())
    metrics = {"uniform": sum(uniform_loss_list) / len(uniform_loss_list),
               "con_loss": sum(contrastive_loss_list) / len(contrastive_loss_list)}
    return metrics


def cal_test_align_uniform(model, dataloader, args=None):
    results = []
    labels = []
    embs = []
    encoder = getattr(model, model.encoder_name)
    ...
    uniform = F.pdist(embs).pow(2).mul(-2).exp().mean().log().item()
...
class CLTrainer(Trainer):
    ...
    #     self.eval_test_pos_dataset = None
    #     self.eval_train_dataset = None
    #     self.eval_test_dataset = None
